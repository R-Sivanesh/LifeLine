...
class Settings(BaseSettings):
    ...
    IS_VERCEL: bool = os.getenv("VERCEL", "").strip() in ("1", "true")
    
    # SQLite fallback location if DATABASE_URL is empty
    SQLITE_DB_PATH: str = "/tmp/lifeline.db" if os.getenv("VERCEL") == "1" else str(Path(__file__).resolve().parent.parent / "lifeline.db")
